...
from database.ia_filterdb import save_file1, save_file2, save_file3, save_file4, save_file5, save_file6, check_file, get_readable_time
...
async def index_files(bot, query):
    ...
    if raju == 'accept1':
        ...
        try:
            ...
            chat = chat
        await index_files_to_db6(int(lst_msg_id), chat, msg, bot)
    elif raju == 'accept6':
        if lock.locked():
            return await query.answer('Wait until previous process complete.', show_alert=True)
        msg = query.message
...
async def index_files_to_db6(lst_msg_id, chat, msg, bot):
    total_files = 0
    duplicate = 0
    no_media = 0
    fst_msg_id = temp.CURRENT
